...
- 部署后可以先打开 `https://你的部署域名/api/health`，如果返回 `{"ok":true,...}`，说明 EdgeOne 云函数已经生效。
- 如果 EdgeOne 函数报体积或超时限制，优先降低单次生成数量，或者改用“浏览器直连”测试是不是代理限制导致。
- 生图模型通常耗时较长，免费函数代理可能先超时。网页已加入兜底：代理遇到 EdgeOne 504 HTML 时会自动尝试浏览器直连一次；如果直连也失败，多半是中转站没有开放 CORS。
